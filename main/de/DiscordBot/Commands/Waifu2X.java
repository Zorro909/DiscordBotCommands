...
public class Waifu2X extends DiscordCommand {
    ...
    public Waifu2X() {
        super("waifu", new String[] { "waifu2x" }, "Upscales a image using the Waifu2X API",
                "Attach a Image to your command message");
    ...
    public Object execute(String command, String[] args, Message m) {
        ...
        try {
            if (a.getHeight() > 2559 || a.getWidth() > 2559) {
                return new MessageBuilder().append("The attached Image must be smaller than 2560*2560 in dimensions!")
                        .build();
            }
            Process p = new ProcessBuilder("curl", "-F", "image=" + a.getUrl(), "-H",
                    "api-key:8b40a9fc-39ea-401c-b656-fa804d380874", "https://api.deepai.org/api/waifu2x").start();
            int seconds = 0;
            ...
            m.getChannel().sendFile(f, new MessageBuilder().append("Your Waifu2x processed Picture!").build())
                    .complete();
            f.delete();
            return "";
        } catch (IOException e) {
