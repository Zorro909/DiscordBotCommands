...
public class OsuProfile extends DiscordCommand {
    ...
    public Object execute(String command, String[] args, Message m) {
        ...
        String countryRank = jo.get("pp_country_rank").getAsString() + " (" + jo.get("country").getAsString() + ")";

        EmbedBuilder stats = new EmbedBuilder();
        stats.setAuthor(username, "https://osu.ppy.sh/users/" + jo.get("user_id").getAsString(),
                "https://a.ppy.sh/" + jo.get("user_id").getAsString() + "?1481308162.png");
        stats.setColor(Color.pink);
        BufferedImage bi = null;
        try {
            bi = ImageIO.read(new URL("https://a.ppy.sh/" + jo.get("user_id").getAsString() + "?1481308162.png"));
        } catch (IOException e) {
            // TODO Auto-generated catch block
            e.printStackTrace();
        }
        ...
        stats.addField("Accuracy", (double) Math.round(Double.valueOf(accuracy) * 10000) / 10000 + "%", true);
