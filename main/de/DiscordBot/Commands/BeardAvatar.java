...
public class BeardAvatar extends DiscordCommand {
    ...
    public BeardAvatar() {
        super("mustacheavatar", new String[] { "mustache", "beard" }, "Puts a mustache on the avatar of the user",
                "\\mustacheavatar {@User}");
    ...
    public Object execute(String command, String[] args, Message m) {
        ...
        if (m.getMentionedUsers().isEmpty()) {
            try {
                ...
                bi.getGraphics().drawImage(beard, bi.getWidth() / 4, bi.getHeight() / 3, bi.getWidth() / 2,
                        bi.getHeight() / 2, null);
                File cache = new File(new Random().nextInt(9999) + ".png");
                ImageIO.write(bi, "png", cache);
                m.getChannel()
                        .sendFile(cache, m.getAuthor().getName() + ".png",
                                new MessageBuilder().append(m.getAuthor().getName() + " with Beard! :joy:").build())
                        .complete();
                cache.delete();
                return "";
            } catch (IOException e) {
            ...
            try {
                ...
                ImageIO.write(bi, "png", cache);
                m.getChannel()
                        .sendFile(cache, m.getMentionedUsers().get(0) + ".png", new MessageBuilder()
