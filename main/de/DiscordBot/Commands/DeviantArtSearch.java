...
public class DeviantArtSearch extends DiscordCommand {
    ...
    public static Object searchForRandomImage(String search) {
        ...
        if (auth == null || auth.getToken().hasExpired()) {
            ClientCredentials CREDENTIALS = new ClientCredentials(6923, "c4e2973fbe94ac4ce8cf3dcd4e69a20d");
        ...
        if (r.isSuccess()) {
            ...
            try {
                ImageIO.write(ImageIO.read(new URL(response.getResults()[rand].getContent().getSource())), "png",
                        cache);
                return cache;
            } catch (IOException e) {
    ...
    public Object execute(String command, String[] args, Message m) {
        if (args.length < 1 || args[0].isEmpty()) {
            return new MessageBuilder().append("You need to supply at least one tag to search for!").build();
        }
        String tag = args[0];
        if (args.length > 1) {
        ...
        if (ca == null) {
            return new MessageBuilder().append("Service Unavailable due to overloading or spamming").build();
        } else if (ca instanceof Integer) {
            return new MessageBuilder().append("There is no image tagged " + tag + " on deviantart! Sorry :(").build();
        } else {
            m.getChannel().sendFile((File) ca, new MessageBuilder().append("Have Fun with your pic!").build())
                    .complete();
            ((File) ca).delete();
        }
        return "";
