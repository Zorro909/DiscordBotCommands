	...
	public Object execute(String command, String[] args, Message m) {
		...
		if(args[0].equalsIgnoreCase("stats")) {
			...
			if(msg.isEmpty()) {
				...
				return new EmbedBuilder().setTitle("Quote from " + m.getMentionedUsers().get(0).getName()).setAuthor(m.getMentionedUsers().get(0).getName()).setThumbnail(m.getMentionedUsers().get(0).getAvatarUrl()).addField("Quote", msg.get(new Random().nextInt(msg.size())),true).build();
			}
		}
		return null;
	}
